...
# o esto. Estas dos líneas son lo mismo
...
select(flights, Origin:Cancelled) # Seleccionar las finlas de Origin a cancelled
select(flights, -(DepTime:AirTime)) # Seleccionar filas penos las que están entre deptime y airtime
select(flights, UniqueCarrier, FlightNum, contains("Tail"), ends_with("Delay"))
select(flights, UniqueCarrier, FlightNum, contains("Tail"), ends_with("Delay"), matches("^Sec")) # El último parámetro es para lo de case sensitive

# MUTATE() ----------------------------------------------------------------------------

# Creamos un DF llamado foo y mutamos del DF flights y creamos una columna
# llamada ActualGroundTime que es igual a lo que aparece
foo <- mutate(flights, ActualGroundTime = ActualElapsedTime - AirTime)

# Añadimos una nueva columna en el DF foo 
foo <- mutate(foo, GroundTime = TaxiIn + TaxiOut)

# Escogemos las columnas que queremos
select(foo, ActualGroundTime, GroundTime)

# Varias operaciones. Si queremos hacer varias operaciones. Aquí estamos creando
# dos nuevas columnas.
foo <- mutate(flights, 
              loss = ArrDelay - DepDelay, 
              loss_percent = (loss/DepDelay) * 100 )
select(foo, loss, loss_percent)
...
# Es opcional, pero así nos hemos creado una variable tipo fecha y podemos operar en
# esa columna con condiciones tipo fecha
...
# Vamos a coger los vuelos cancelados con esas columnas
# Además vamos a coger aquellas filas que que no tienen NA en DepDelay
...
arrange(cancelled, Dest, desc(DepDelay))
rm(cancelled)
...
summarise(taxi, max_diff = max(abs(TaxiIn - TaxiOut)))
rm(taxi)
...
# Aquí lo que hacemos es eliminar de mi data set aquellos valores que están 
# consideradas estadísticamente como outlier (valores extremos)
# lo hacemos con el $out
...
# Con table nos saca las frecuencias
# En este caso nos saca el número de vuelos por compañía
...
# Esta librería es para limpieza de datos
...
# Convertir los NA a cero. 0L forzamos que sea un número entero
# Coalesce es para forzar a la columna
flights %>% 
...
# spread
# gather
# Lo de spread y lo de gather es para pivotar la tabla.
# Con spread hemos pivotado la tabla
# Con gather hemos hecho lo contrario, En caso de que nos encontremos
# la tabla pivotada pues la utilizamos. Es dispovitar
...
# Con unite aquí hemos creado una columna y le hemos dado el valor
# de dos columnas y le ponemos la separación de "-"

# Con separate podemos crear columnas nuevas con valores de una columna
# y separamos ese valor de la columna. Tiene un parámetro para decirle
# el separador, pero la propia función la puede interpretar
...
# Esto es para ver si tenemos elementos duplicados en la tabla
airports %>% 
  count(iata) %>% 
  filter(n > 1)
...
foo %>% ggplot(aes(x=lat, y=long)) + geom_point()
...
flights %>% 
  ...
  separate(DepTime, into = c("Hour", "Minute"), sep = -2, remove = F)

flights %>% 
  head %>%
  select(Year:DayofMonth,DepTime,ArrTime) %>% 
  separate(DepTime, into = c("Hour", "Minute"), sep = -2) %>% 
